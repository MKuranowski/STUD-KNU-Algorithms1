...
class DijkstraSearchEntry:
    ...
    def __lt__(self, o: "DijkstraSearchEntry") -> bool:
        ...
        if self.nodes_to_visit == o.nodes_to_visit and self.nodes_visited == o.nodes_visited:
            return self.fuel_used < o.fuel_used
...
class DijkstraSearch:
    ...
    def do(self) -> tuple[float, list[int]]:
        ...
        while self.queue:
            ...
            # End reached
            if entry.point == end and (self.length is None or entry.nodes_visited == self.length):
            ...
            for next in range(entry.point+1, len(self.pts)):
                next_entry = DijkstraSearchEntry(
                    ...
                    fuel_used=entry.fuel_used + self.distances[entry.point][next],
                )

                # Skip not permitted entries
                if next_entry.fuel_used > self.max_fuel or (
                    self.length is not None and next_entry.nodes_visited > self.length
                ):
                    continue
                ...
                if existing and not existing.deleted and next_entry.fuel_used > existing.fuel_used:
